...
FullRegIntl = OLS(nIntlRet, regDF.iloc[-NINTL:]).fit()
print('Full Regression for International Stocks')
print(FullRegIntl.summary())
...
statDF = pd.DataFrame({'reg' : allNames, 'length' : lengths, 'std' : stdAll, 'skew': skewAll, 'kurt' : kurtAll, 'SW' : SWp, 'JB' : JBp, 'L1O': L1O, 'L1A' : L1A})
print(statDF)
