...
# and apply Box-Cox transform to exponentiated residuals
volresid = RegVol.resid
nresid = scipy.stats.boxcox(np.exp(volresid))[0]
la = scipy.stats.boxcox(np.exp(volresid))[1]
print('lambda = ', la)
...
# now apply the Box-Cox transform to the volatility data
nvol = scipy.stats.boxcox(vol)[0]
la = scipy.stats.boxcox(np.exp(nvol))[1]
print('lambda = ', la)
